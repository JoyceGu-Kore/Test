...
var sendMsg = require("../../visualIVR.js");
...
function loadroutes(app) {
    ...
    app.post(apiPrefix + '/send', sendMsg.claimNotifier);
    app.get(apiPrefix + '/gethistory', function(req, res) {
        sendMsg.gethistory(req.query.userId).then(function(data) {
            res.status(200).send(data);
        }).catch(function(err) {
            res.status(401).send("Error in getting history : " + err.message);
        })
    });
}
